#ifndef CORE_HAMMER_HPP__
#define CORE_HAMMER_HPP__
...
//  template< typename Params, typename State = Eigen::VectorXd, typename Action = Eigen::VectorXd,typename Selector = ConfSelector<Params>, typename ConfUpdator=Dist>
  ...
  public:
    ...
    void run( System& system, const state_t& target,const state_t& initState, bool disp_pred=false)
    {
    ...
    void learn( System& system, const state_t& target, const state_t& initState) // IMITATION LEARNING -->>> EXPERIMENTAL
    {
      ...
      do{
	this->_update_inversePredictions(_currentState, target);
	this->_update_forwardPredictions(_currentState, target);
	//action_t selectedAction; // same here, how do we know the executed action;
	state_t newState=system(); 
	//this->print_prediction();
	this->_update_confidence(newState); // the performed action is unknown
    ...
    void updateModels(const state_t& prevState, const action_t& selectedAction, const state_t& newState)
    {
      this->_update_confidence(newState, selectedAction);
      this->_update_models(prevState, selectedAction, newState);
      _currentState=newState;
    }
  ...
  private:
    ...
    bool _stop(const HMR& hmr,const state_t& target) const
    {
      hammer::stop::ChainCriteria<HMR> chain(hmr,target);
	...
	return;
      _res_dir = ((std::string)Params::hammer::name())+"_"+tools::hostname() + "_" + tools::date() + "_" + tools::getpid();
    ...
    void _update_forwardPredictions(const state_t& current, const state_t& target) 
    {
      tbb::parallel_for_each(_forwardModels.begin(),_forwardModels.end(), [=](std::shared_ptr<ModelPair_t >& fm ) {(*fm)(current); (*fm).updateScore( target );} );
    }
    ...
    void _update_confidence(const state_t& state, const action_t action)
    {
      tbb::parallel_for_each(_forwardModels.begin(),_forwardModels.end(), [=](std::shared_ptr<ModelPair_t >& fm ) {(*fm).udpateConfidence(state,action);} );
    }
